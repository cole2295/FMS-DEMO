...
namespace RFD.FMS.DAL.BasicSetting
{
   ...
   * 修改记录：
   */
    public class ExpressPriceDao : SqlServerDao, IExpressPriceDao
    {
        #region SQLSTRING
        ...
        private const string strExpressList = @"SELECT a.AreaID,a.AreaName,a.CityID,a.PostCode,a.ZoneCode,a.TransferFee,a.Sorting,a.IsDeleted,
                                ...
                                FROM RFD_PMS.dbo.Area a(NOLOCK) inner join RFD_PMS.dbo.City c(NOLOCK) on a.CityID = c.cityid
                                WHERE a.CityID in (SELECT cityID FROM RFD_PMS.dbo.City c(NOLOCK) WHERE c.IsDeleted <> 1)";
        #endregion
        ...
        public DataTable GetAreaInfo(string strCityID, string strCityName)
        {
            ...
            DataTable dt = SqlHelper.ExecuteDataset(ReadOnlyConnection, CommandType.Text, strSql).Tables[0];
            return dt;
        }
    }
}
