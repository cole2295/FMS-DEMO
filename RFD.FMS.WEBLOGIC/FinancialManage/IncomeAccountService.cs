...
namespace RFD.FMS.WEBLOGIC.FinancialManage
{
	public class IncomeAccountService : IIncomeAccountService
	{
		...
		private IIncomeAccountService OracleSerivce;

		/// <summary>
		/// 查询日统计表
		/// </summary>
		/// <param name="IncomeSearchCondition"></param>
		/// <returns></returns>
		public List<IncomeAccountDetail> GetUniteAccount(IncomeSearchCondition IncomeSearchCondition)
		{
			if (OracleSerivce != null)
			{
				return OracleSerivce.GetUniteAccount(IncomeSearchCondition);
			}
			else
			{
		...
		private List<IncomeAccountDetail> TransformToDetailModel(DataTable dt)
		{
			...
			var list = new List<IncomeAccountDetail>();
			foreach (DataRow dr in dt.Rows)
			{
				var m = new IncomeAccountDetail();
		...
		/// <returns></returns>
		public bool CreateIncomeAccount(IncomeSearchCondition condition, List<IncomeAccountDetail> list, int createBy,
		                                out string AccountNo)
		{
			if (OracleSerivce != null)
			{
				return OracleSerivce.CreateIncomeAccount(condition, list, createBy, out AccountNo);
			}
			else
			{
				var noGenerate = ServiceLocator.GetService<INoGenerate>();
		...
		public void BuildAccountDetail(ref List<IncomeAccountDetail> list)
		{
			...
			var statExpresscompany =
				...
				group t1 by new {t1.ExpressCompanyID}
				into g
			...
			var all = new IncomeAccountDetail()
			          	{
			          		...
			          		Fare = statDetail.Select(t => t.DeliveryFare).Sum() + statDetail.Select(t => t.DeliveryVFare).Sum() +
			          		       ...
			          		       statDetail.Select(t => t.VisitReturnsFare).Sum() +
			          		       statDetail.Select(t => t.VisitReturnsVFare).Sum() +
			          		       statDetail.Select(t => t.ProtectedFee).Sum() + statDetail.Select(t => t.ReceiveFee).Sum() +
			          		       statDetail.Select(t => t.ReceivePOSFee).Sum(),
			          		DataType = 2,
			          	};
		...
		public DataTable GetAccountList(string accountStatus, string merchantId, string dateStr, string dateEnd,
		                                string accountNo)
		{
			if (OracleSerivce != null)
			{
				return OracleSerivce.GetAccountList(accountStatus, merchantId, dateStr, dateEnd, accountNo);
			}
			else
			{
		...
		public List<IncomeAccountDetail> GetAccountDetails(string accountNo)
		{
			if (OracleSerivce != null)
			{
				return OracleSerivce.GetAccountDetails(accountNo);
			}
			else
			{
		...
		public List<IncomeAccountDetail> GetAccountDetailsNew(string accountNo)
		{
			if (OracleSerivce!=null)
			{
				return OracleSerivce.GetAccountDetailsNew(accountNo);
			}
			else
			{
		...
		public IncomeAccountDetail GetAccountDetail(string detailId)
		{
			if (OracleSerivce!=null)
			{
				return OracleSerivce.GetAccountDetail(detailId);
			}
			else
			{
		...
		public IncomeAccountDetail GetAccountDetailByAccountNo(string accountNo)
		{
			if (OracleSerivce!=null)
			{
				return	OracleSerivce.GetAccountDetailByAccountNo(accountNo);
		...
		public IncomeSearchCondition GetAccountSearchCondition(string accountNo)
		{
			if (OracleSerivce != null)
			{
				return OracleSerivce.GetAccountSearchCondition(accountNo);
			}
			else
			{
		...
		public bool DeleteAccount(IList<KeyValuePair<string, string>> checkList, int updateBy)
		{
			try
			{
				if (OracleSerivce!=null)
				{
					return OracleSerivce.DeleteAccount(checkList, updateBy);
				}
				else
				{
					using (IUnitOfWork work = TranScopeFactory.CreateUnit())
		...
		public bool UpdateAccountFee(IncomeAccountDetail i)
		{
			try
			{
				if (OracleSerivce!=null)
				{
					return OracleSerivce.UpdateAccountFee(i);
				}
				else
				{
					using (IUnitOfWork work = TranScopeFactory.CreateUnit())
		...
		public bool UpdateAccountFeeByAccountNo(IncomeAccountDetail i)
		{
			try
			{
				if (OracleSerivce!=null)
				{
					return OracleSerivce.UpdateAccountFeeByAccountNo(i);
				}
				else
				{
					using (IUnitOfWork work = TranScopeFactory.CreateUnit())
		...
		public bool UpdateAccountStatus(IList<KeyValuePair<string, string>> list, int auditBy, int status)
		{
			try
			{
				if (OracleSerivce!=null)
				{
					return OracleSerivce.UpdateAccountStatus(list,auditBy,status);
				}
				else
				{
					using (IUnitOfWork work = TranScopeFactory.CreateUnit())
					{
						if (list.Any(k => !_incomeAccountDao.UpdateAccountStatus(k.Key, auditBy, status)))
						{
							return false;
						}
						work.Complete();
		...
		{
			var dt = OracleSerivce != null
			         	? OracleSerivce.GetAccountAreaSummary(accountNo)
			         	: _incomeAccountDao.GetAccountAreaSummary(accountNo);
            return OracleSerivce ==null?DisposeTotalAreaSummary(dt):dt;
		}
		...
		private DataTable DisposeTotalAreaSummary(DataTable dt)
		{
			...
			foreach (DataRow dr in dt.Rows)
			{
				...
				drTotal["DeliveryNum"] = drTotal["DeliveryNum"].ToString().TryGetDecimal() +
				                         dr["DeliveryNum"].ToString().TryGetDecimal();
				drTotal["DeliveryFare"] = drTotal["DeliveryFare"].ToString().TryGetDecimal() +
				                          dr["DeliveryFare"].ToString().TryGetDecimal();
				drTotal["ReturnsVNum"] = drTotal["ReturnsVNum"].ToString().TryGetDecimal() +
				                         dr["ReturnsVNum"].ToString().TryGetDecimal();
				drTotal["DeliveryVFare"] = drTotal["DeliveryVFare"].ToString().TryGetDecimal() +
				                           dr["DeliveryVFare"].ToString().TryGetDecimal();
				drTotal["ProtectedFee"] = drTotal["ProtectedFee"].ToString().TryGetDecimal() +
				                          dr["ProtectedFee"].ToString().TryGetDecimal();
				drTotal["ReceiveFee"] = drTotal["ReceiveFee"].ToString().TryGetDecimal() +
				                        dr["ReceiveFee"].ToString().TryGetDecimal();
				drTotal["ReceivePOSFee"] = drTotal["ReceivePOSFee"].ToString().TryGetDecimal() +
				                           dr["ReceivePOSFee"].ToString().TryGetDecimal();
        ...
        public DataTable GetMerchantInfo(string distributionCode)
		{
			if (OracleSerivce != null)
			{
                return OracleSerivce.GetMerchantInfo(distributionCode);
			}
			else
			{
		...
		/// <returns></returns>
		public DataTable GetIncomeDeliveryAccount(int merchantId, DateTime accountDate, int deliverstationid,
		                                          int expresscompanyid)
		{
			if (OracleSerivce != null)
			{
				return OracleSerivce.GetIncomeDeliveryAccount(merchantId, accountDate, deliverstationid, expresscompanyid);
			}
			else
			{
		...
		public DataTable GetIncomeDeliveryAccountInfo(int merchantId, DateTime accountDate)
		{
			if (OracleSerivce != null)
			{
				return OracleSerivce.GetIncomeDeliveryAccountInfo(merchantId, accountDate);
			}
			else
			{
		...
		/// <returns></returns>
		public int CollateIncomeDeliveryAccountInfo(int merchantid, DateTime accountDate, int warehouseid,
		                                            int deliverstationid)
		{
			if (OracleSerivce != null)
			{
				return OracleSerivce.CollateIncomeDeliveryAccountInfo(merchantid, accountDate, warehouseid, deliverstationid);
			}
			else
			{
		...
		public int AddIncomeDeliveryAccount(IncomeDeliveryCount model, IncomeStatLog modelLog)
		{
			if (OracleSerivce != null)
			{
				...
				{
					OracleSerivce.AddIncomeDeliveryAccount(model, modelLog);
					OracleSerivce.AddIncomeStatLog(modelLog);
					work.Complete();
					return 1;
				}
			}
		...
		public int AddIncomeStatLog(IncomeStatLog model)
		{
			if (OracleSerivce != null)
			{
				return OracleSerivce.AddIncomeStatLog(model);
			}
			else
			{
		...
		public bool UpdateIncomeStatLog(IncomeStatLog model)
		{
			if (OracleSerivce != null)
			{
				return OracleSerivce.UpdateIncomeStatLog(model);
			}
			else
			{
		...
		public bool AddIncomeDeliveryDetails(int merchantId, DateTime countDate, int deliverstationid, int expresscompanyid)
		{
			if (OracleSerivce != null)
			{
				return OracleSerivce.AddIncomeDeliveryDetails(merchantId, countDate, deliverstationid, expresscompanyid);
			}
			else
			{
				...
				if (dt.Rows.Count > 0)
				{
					var model = new IncomeDeliveryCount();
		...
		public DataTable GetIncomeReturnsCount(int merchantId, DateTime accountDate)
		{
			if (OracleSerivce != null)
			{
				return OracleSerivce.GetIncomeReturnsCount(merchantId, accountDate);
			}
			else
			{
		...
		public int CollateIncomeReturnsAccountInfo(int merchantid, DateTime accountDate, int warehouseid, int deliverstationid)
		{
			if (OracleSerivce != null)
			{
				return OracleSerivce.CollateIncomeReturnsAccountInfo(merchantid, accountDate, warehouseid, deliverstationid);
			}
			else
			{
		...
		/// <param name="merchantId"></param>
		/// <param name="countDate"></param>
		public bool AddIncomeReturnsAccountDetails(int merchantId, DateTime countDate, int deliverStationId,
		                                           int returnExpressId)
		{
			if (OracleSerivce != null)
			{
				return OracleSerivce.AddIncomeReturnsAccountDetails(merchantId, countDate, deliverStationId, returnExpressId);
			}
			else
			{
		...
		public DataTable GetIncomeVisitReturnsCount(int merchantId, DateTime accountDate)
		{
			if (OracleSerivce != null)
			{
				return OracleSerivce.GetIncomeVisitReturnsCount(merchantId, accountDate);
			}
			else
			{
		...
		/// <param name="deliverstationid"></param>
		/// <returns></returns>
		public int CollateIncomeVisitReturnsAccountInfo(int merchantid, DateTime accountDate, int warehouseid,
		                                                int deliverstationid)
		{
			if (OracleSerivce != null)
			{
				return OracleSerivce.CollateIncomeVisitReturnsAccountInfo(merchantid, accountDate, warehouseid, deliverstationid);
			}
			else
			{
		...
		/// <param name="countDate"></param>
		/// <returns></returns>
		public bool AddIncomeVisitReturnsAccountDetails(int merchantId, DateTime countDate, int deliverStationId,
		                                                int returnExpressId)
		{
			if (OracleSerivce != null)
			{
				return OracleSerivce.AddIncomeVisitReturnsAccountDetails(merchantId, countDate, deliverStationId, returnExpressId);
			}
			else
			{
				//收入结算--上门（站点，返回分拣中心，区域类型，上门退类型，结算标准）
				DataTable dt = _incomeAccountDao.GetIncomeVisitReturnsAccount(merchantId, countDate, deliverStationId,
				                                                              returnExpressId);
		...
		public DataTable GetIncomeOtherReceiveFee(int merchantId, DateTime accountDate)
		{
			if (OracleSerivce != null)
			{
				return OracleSerivce.GetIncomeOtherReceiveFee(merchantId, accountDate);
			}
			else
			{
		...
		public int CollateIncomeOtherReceiveFee(int merchantid, DateTime accountDate, int deliverstationid)
		{
			if (OracleSerivce != null)
			{
				return OracleSerivce.CollateIncomeOtherReceiveFee(merchantid, accountDate, deliverstationid);
			}
			else
			{
		...
		public bool AddIncomeOtherReceiveFee(int merchantId, DateTime countDate, int deliverStationId)
		{
			if (OracleSerivce != null)
			{
				return OracleSerivce.AddIncomeOtherReceiveFee(merchantId, countDate, deliverStationId);
			}
			else
			{
				...
				if (dt.Rows.Count > 0)
				{
					...
					//using (IUnitOfWork work = TranScopeFactory.CreateUnit())
					{
						for (int i = 0; i < dt.Rows.Count; i++)
						{
							...
							model.CreateBy = 0;
							model.CreateTime = DateTime.Now;
							model.ReceiveFee = string.IsNullOrEmpty(dt.Rows[i]["Fare"].ToString())
							                   	? 0
							                   	: decimal.Parse(dt.Rows[i]["Fare"].ToString());
							model.ReceiveStandard = string.IsNullOrEmpty(dt.Rows[i]["Formula"].ToString())
							                        	? 0
							                        	: decimal.Parse(dt.Rows[i]["Formula"].ToString());
							model.ReceivePOSFee = string.IsNullOrEmpty(dt.Rows[i]["POSFare"].ToString())
							                      	? 0
							                      	: decimal.Parse(dt.Rows[i]["POSFare"].ToString());
							model.ReceivePOSStandard = string.IsNullOrEmpty(dt.Rows[i]["POSFormula"].ToString())
							                           	? 0
							                           	: decimal.Parse(dt.Rows[i]["POSFormula"].ToString());
							model.ServesFee = string.IsNullOrEmpty(dt.Rows[i]["ServiceFare"].ToString())
							                  	? 0
							                  	: decimal.Parse(dt.Rows[i]["ServiceFare"].ToString());
							model.ServesStandard = string.IsNullOrEmpty(dt.Rows[i]["ServiceFormula"].ToString())
							                       	? 0
							                       	: decimal.Parse(dt.Rows[i]["ServiceFormula"].ToString());
							model.POSServesFee = string.IsNullOrEmpty(dt.Rows[i]["POSServiceFare"].ToString())
							                     	? 0
							                     	: decimal.Parse(dt.Rows[i]["POSServiceFare"].ToString());
							model.POSServesStandard = string.IsNullOrEmpty(dt.Rows[i]["POSServiceFormula"].ToString())
							                          	? 0
							                          	: decimal.Parse(dt.Rows[i]["POSServiceFormula"].ToString());
							model.IsDeleted = 0;
							model.MerchantID = merchantId;
							model.StationID = int.Parse(dt.Rows[i]["DeliverStationID"].ToString());
							model.UpdateBy = 0;
		...
		public DataTable GetIncomeHistoryCount(DateTime accountDate)
		{
			if (OracleSerivce!=null)
			{
				return OracleSerivce.GetIncomeHistoryCount(accountDate);
			}
			else
			{
		...
		public int IncomeDeliveryAccountHistory(int merchantid, DateTime accountDate, int warehouseid, int deliverstationid)
		{
			if (OracleSerivce!=null)
			{
				return OracleSerivce.IncomeDeliveryAccountHistory(merchantid, accountDate, warehouseid, deliverstationid);
			}
			else
			{
		...
		public int IncomeReturnsAccountHistory(int merchantid, DateTime accountDate, int warehouseid, int deliverstationid)
		{
			if (OracleSerivce!=null)
			{
				return OracleSerivce.IncomeReturnsAccountHistory(merchantid, accountDate, warehouseid, deliverstationid);
			}
			else
			{
		...
		public int IncomeVisitReturnsAccountHistory(int merchantid, DateTime accountDate, int warehouseid,
		                                            int deliverstationid)
		{
			if (OracleSerivce!=null)
			{
				return OracleSerivce.IncomeVisitReturnsAccountHistory(merchantid,accountDate,warehouseid,deliverstationid);
			}
			else
			{
		...
		public int IncomeOtherReceiveFeeHistory(int merchantid, DateTime accountDate, int deliverstationid)
		{
			if (OracleSerivce!=null)
			{
				return OracleSerivce.IncomeOtherReceiveFeeHistory(merchantid,accountDate,deliverstationid);
			}
			else
			{
		...
		public int IsIncomeHistoryCount(DateTime accountDate)
		{
			if (OracleSerivce != null)
			{
				return OracleSerivce.IsIncomeHistoryCount(accountDate);
			}
			else
			{
