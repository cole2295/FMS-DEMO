...
using RFD.FMS.Domain.BasicSetting;using System.Data.SqlClient;
...
namespace RFD.FMS.WEBLOGIC.FinancialManage
{
    public class WaybillForIncomeBaseInfoService : IWaybillForIncomeBaseInfoService, IWaybillStatusObserver
    {
        ...
        private IWaybillForIncomeBaseInfoService OracleService;

        //SQL
        //IAreaExpressLevelIncomeDao areaExpressLevelIncomeDao = new RFD.FMS.DAL.BasicSetting.AreaExpressLevelIncomeDao();
        //ORACLE
        IAreaExpressLevelIncomeDao areaExpressLevelIncomeDao = new RFD.FMS.DAL.Oracle.BasicSetting.AreaExpressLevelIncomeDao();
        ...
        public DataTable SearchDetailsV2(ThirdPartyWaybillSearchConditons condition, ref PageInfo pi, out DataTable amount)
        {
            ...
            {
                return OracleService.SearchDetailsV2(condition, ref pi, out amount);
            }
            string searchCondition = BuildSearchCondition(condition);
            List<SqlParameter> parameterList = new List<SqlParameter>();
            amount = _waybillForIncomeBaseInfoDao.SearchStatV2<SqlParameter>(searchCondition, parameterList);
            ...
            return _waybillForIncomeBaseInfoDao.SearchDetailsV2<SqlParameter>(searchCondition,parameterList, ref pi, true);
        }
        ...
        public DataTable SearchDetails(ThirdPartyWaybillSearchConditons condition,ref PageInfo pi,out DataTable amount)
        {
            if(OracleService !=null)
            {
                return OracleService.SearchDetails(condition, ref pi, out amount);
            }
            string searchCondition = BuildSearchCondition(condition);
            List<SqlParameter> parameterList = new List<SqlParameter>();
            amount = _waybillForIncomeBaseInfoDao.SearchStat<SqlParameter>(searchCondition,parameterList);

            if (amount == null || amount.Rows.Count <= 0 || int.Parse(amount.Rows[0]["订单量合计"].ToString()) <= 0) return null;

            pi.ItemCount = int.Parse(amount.Rows[0]["订单量合计"].ToString());

            return _waybillForIncomeBaseInfoDao.SearchDetails<SqlParameter>(searchCondition,parameterList, ref pi, true);
        }

        private string BuildSearchCondition(ThirdPartyWaybillSearchConditons condition)
        {
            StringBuilder sbCondition = new StringBuilder();
            sbCondition.Append(string.Format(" AND fibi.DistributionCode = '{0}' ", condition.DistributionCode));
            if(condition.DistributionCode=="rfd")
                sbCondition.Append(" AND fibi.MerchantID NOT IN (8,9) ");
            sbCondition.Append(" AND fifi.IsDeleted = 0 ");
            if (!string.IsNullOrEmpty(condition.WaybillNO.ToString()) && condition.WaybillNO > 0)
            {
                sbCondition.Append(string.Format(" AND fibi.WaybillNo = {0}", condition.WaybillNO));
            }
            else if (!string.IsNullOrEmpty(condition.Customerorder))
            {
                sbCondition.Append(string.Format(" AND fibi.CustomerOrder = '{0}'", condition.Customerorder));
            }
            else
            {
                if (!string.IsNullOrEmpty(condition.MerchantID))
                    sbCondition.Append(string.Format(" AND fibi.MerchantID IN ({0}) ", condition.MerchantID));

                if (!string.IsNullOrEmpty(condition.OutCreatTimeBegin.ToString()))
                    sbCondition.Append(string.Format(" AND fibi.RfdAcceptTime >= '{0}' ", condition.OutCreatTimeBegin));

                if (!string.IsNullOrEmpty(condition.OutCreatTimeEnd.ToString()))
                    sbCondition.Append(string.Format(" AND fibi.RfdAcceptTime < '{0}' ", condition.OutCreatTimeEnd));

                if (!string.IsNullOrEmpty(condition.InCreatTimeBegin.ToString()))
                    sbCondition.Append(string.Format(" AND fibi.ReturnTime >= '{0}' ", condition.InCreatTimeBegin));

                if (!string.IsNullOrEmpty(condition.InCreatTimeEnd.ToString()))
                    sbCondition.Append(string.Format(" AND fibi.ReturnTime < '{0}' ", condition.InCreatTimeEnd));

                if (!string.IsNullOrEmpty(condition.BackStationTimeBegin.ToString()))
                    sbCondition.Append(string.Format(" AND fibi.BackStationTime >= '{0}' ", condition.BackStationTimeBegin));

                if (!string.IsNullOrEmpty(condition.BackStationTimeEnd.ToString()))
                    sbCondition.Append(string.Format(" AND fibi.BackStationTime < '{0}' ", condition.BackStationTimeEnd));

                if (!string.IsNullOrEmpty(condition.SortingCenter))
                    sbCondition.Append(string.Format(" AND fibi.ExpressCompanyID IN ({0}) ", condition.SortingCenter));

                //使用结算主体查询
                if (!string.IsNullOrEmpty(condition.DeliverStationID))
                    sbCondition.Append(string.Format(" AND fibi.TopCODCompanyID IN ({0}) ", condition.DeliverStationID));

                if (!string.IsNullOrEmpty(condition.WaybillStatus))
                    sbCondition.Append(string.Format(" AND fibi.BackStationStatus = '{0}' ", condition.WaybillStatus));

                if (!string.IsNullOrEmpty(condition.BackStatus))
                    sbCondition.Append(string.Format(" AND fibi.SubStatus = {0} ", Convert.ToInt32(condition.BackStatus)));

                if (!string.IsNullOrEmpty(condition.WaybillType))
                    sbCondition.Append(string.Format(" AND fibi.WaybillType = '{0}' ", condition.WaybillType));

                if (!string.IsNullOrEmpty(condition.InefficacyStatus.ToString()))
                    sbCondition.Append(string.Format(" AND fibi.InefficacyStatus = {0} ", condition.InefficacyStatus));

                if (!string.IsNullOrEmpty(condition.PaymentType))
                    sbCondition.Append(string.Format(" AND fibi.AcceptType = '{0}' ", condition.PaymentType));
                ...
                if (!string.IsNullOrEmpty(condition.AreaExpressLevel.ToString()))
                {
                    if (condition.AreaExpressLevel == 99)
                    {
                        ...
                        sbCondition.Append(string.Format(" AND fifi.AreaType={0}", condition.AreaExpressLevel.ToString()));
                }
            }
        ...
        public DataTable SearchDetailsForExportV2(ThirdPartyWaybillSearchConditons condition)
        {
            if(OracleService !=null)
            {
                return OracleService.SearchDetailsForExportV2(condition);
            }
            string searchCondition = BuildSearchCondition(condition);
            List<SqlParameter> parameterList = new List<SqlParameter>();
            PageInfo pi = new PageInfo(0);

            return _waybillForIncomeBaseInfoDao.SearchDetailsV2<SqlParameter>(searchCondition,parameterList, ref pi, false);
        }
        ...
        public DataTable SearchDetailsForExport(ThirdPartyWaybillSearchConditons condition)
        {
            if (OracleService != null)
            {
                return OracleService.SearchDetailsForExport(condition);
            }
            string searchCondition = BuildSearchCondition(condition);
            List<SqlParameter> parameterList = new List<SqlParameter>();
            PageInfo pi = new PageInfo(0);
            return _waybillForIncomeBaseInfoDao.SearchDetails<SqlParameter>(searchCondition,parameterList, ref pi, false);
        }
        ...
        public DataTable SearchSummary(ThirdPartyWaybillSearchConditons condition,out DataTable amount)
        {
            if(OracleService != null)
            {
                return OracleService.SearchSummary(condition, out amount);
            }
            string searchCondition = BuildSearchCondition(condition);
            List<SqlParameter> parameterList = new List<SqlParameter>();
            amount = _waybillForIncomeBaseInfoDao.SearchStat<SqlParameter>(searchCondition, parameterList);
            ...
            return _waybillForIncomeBaseInfoDao.SearchSummary<SqlParameter>(searchCondition, parameterList, condition);
        }
        ...
        public DataTable SearchSummaryForExport(ThirdPartyWaybillSearchConditons condition)
        {
            if (OracleService != null)
            {
                return OracleService.SearchSummaryForExport(condition);
            }
            string searchCondition = BuildSearchCondition(condition);
            List<SqlParameter> parameterList = new List<SqlParameter>();
            return _waybillForIncomeBaseInfoDao.SearchSummary<SqlParameter>(searchCondition, parameterList, condition);
        ...
        public bool DoAction(WaybillStatusChangeLog model)
        {
            ...
            {
                SendFailedMail("收入推数异常", ex.Message+ex.StackTrace);
                return false;
            }
        }
        ...
        public int InsertIntoInComeBaseInfo(WaybillStatusChangeLog Logmodel)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (BaseModel.Status == "3" || BaseModel.Status == "5")
                    {
                        if (string.IsNullOrEmpty(BaseModel.BackStationStatus) || string.IsNullOrEmpty(BaseModel.BackStationTime.ToString()))
                        {
                            var mailService = ServiceLocator.GetService<IMail>();
                            string msg = "归班没有获取到状态：" + BaseModel.WaybillNo;
                            mailService.SendMailToUser("收入推数异常", msg, "zengwei@vancl.cn");
                        }
                    }
        ...
        private int GetAreaType(int merchantId, string wareHouseId, string distributionCode, string areaId, int isCategory, string waybillCategory,int? expressId)
        {
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                if (expressId != null && expressId > 0)
                {
                    ...
                    {
                        DataRow[] dataRowsk = dt.Select("EXPRESSCOMPANYID='" + 11 + "'");
                        if (dataRowsk.Length > 0)
                        {
                            return int.Parse(dataRowsk[0]["AREATYPE"].ToString());
                        }
                    }
                }
                else
        ...
        /// <param name="distributionCode"></param>
        /// <returns></returns>
        private KeyValuePair<string, int> GetIncomeBasicDeliverFee(int merchantId, string warehouseId, int areaType, int isCategory, string waybillCategory, string distributionCode, decimal needPayAmount)
        {
            //全部类型只要有应收金额，按区分COD来找公式
            KeyValuePair<string, int> result=new KeyValuePair<string,int>();
            IMerchantDeliverFeeDao merchantDeliverFeeDao = ServiceLocator.GetService<IMerchantDeliverFeeDao>();
            ...
            if (dtFee == null || dtFee.Rows.Count !=1)
            {
                return new KeyValuePair<string, int>("",-1);
            }
            
            if (needPayAmount > 0)
                result=new KeyValuePair<string, int>(dtFee.Rows[0]["BasicDeliverFee"].ToString(), DataConvert.ToInt(dtFee.Rows[0]["IsCod"].ToString()));
            else
                result = new KeyValuePair<string, int>(dtFee.Rows[0]["DeliverFee"].ToString(), DataConvert.ToInt(dtFee.Rows[0]["IsCod"].ToString()));
            return result;
        }
        ...
        private string JudgeGetIncomeFeeMsg(DataRow row,ref FMS_IncomeFeeInfo model)
        {
            ...
            //
            //KeyValuePair<string,int> feeResult= GetIncomeBasicDeliverFee(DataConvert.ToInt(row["MerchantID"]), row["ExpressCompanyID"].ToString(), DataConvert.ToInt(model.AreaType), isCategory, row["WaybillCategory"].ToString(), row["DistributionCode"].ToString(),needPayAmount);
        ...
        private string AccountFare(DataRow row, ref FMS_IncomeFeeInfo model)
        {
            ...
            try
            {
                    ...
                    model.IsAccount != (int)BizEnums.IncomeFeeAccountType.T6)
                {
                    ...
                    if (waybillType == "0" && backStatus == "3")
                    {
                        //妥投
                        model.AccountStandard = model.AccountStandard;
                    }
                    else if ((waybillType == "0" || waybillType == "1") && subStatus == "7")
                    {
        ...
        #region 生效服务 OracleService
        
        public void DisposeEffect(int rowCount)
        {
            try
            {
                var merchantDeliverFee = ServiceLocator.GetService<IMerchantDeliverFee>();
