...
namespace RFD.FMS.WEBLOGIC.BasicSetting
{
	...
	{
		private IDeliveryPriceService OracleService;
		...
		public DataTable GetMerchant(string disCode)
		{
			...
			{
				return OracleService.GetMerchant(disCode);
			}
		...
		public DataTable GetDeliveryPriceList(string expressCompanyId, string lineStatus, string auditStatus, string areaType, string wareHouse, string wareHouseType, bool waitEffect, string merchantId, string distributionCode, int IsCod, ref PageInfo pi, bool isPage)
		{
			if (OracleService != null)
			{
				return OracleService.GetDeliveryPriceList(expressCompanyId, lineStatus, auditStatus, areaType, wareHouse, wareHouseType, waitEffect, merchantId, distributionCode, IsCod, ref pi, isPage);
			}
		...
		public FMS_CODLine GetListByCodLineNo(string codLineNo)
		{
			if (OracleService != null)
			{
				return OracleService.GetListByCodLineNo(codLineNo);
			}
		...
		public int AddDeliveryPrice(FMS_CODLine codLine)
		{
            if (OracleService != null)
            {
                return OracleService.AddDeliveryPrice(codLine);
            }
			...
			try
			{
				...
				using (IUnitOfWork work = TranScopeFactory.CreateUnit())
				{
					...
					n = _deliveryPriceDao.AddDeliveryPrice(codLine, cODLineNO);
				...
				//if (OracleService != null)
				//{
				//    OracleService.AddDeliveryPrice(codLine);
				//}
				return n;
			}
			catch (Exception ex)
		...
		public bool UpdateDeliveryPrice(FMS_CODLine codLine)
		{
            if (OracleService != null)
            {
                return OracleService.UpdateDeliveryPrice(codLine);
            }
			...
			try
			{
				...
				//if (OracleService!=null)
				//{
				//    OracleService.UpdateDeliveryPrice(codLine);
				//}
				//return true;
			}
			catch (Exception ex)
		...
		public bool DeleteDeliveryPrice(IList<KeyValuePair<string, string>> keyValuePairs, string deleteBy)
		{
            if (OracleService != null)
            {
                return OracleService.DeleteDeliveryPrice(keyValuePairs, deleteBy);
            }
			...
			try
			{
				...
				//if (OracleService!=null)
				//{
				//    OracleService.DeleteDeliveryPrice(keyValuePairs, deleteBy);
				//}
				return true;
			}
			catch (Exception ex)
		...
		public bool UpdateDeliveryPriceAuditStatus(IList<KeyValuePair<string, string>> keyValuePairs, string auditBy, int auditStatus)
		{
            if (OracleService != null)
            {
                return OracleService.UpdateDeliveryPriceAuditStatus(keyValuePairs, auditBy, auditStatus);
            }
			try
			{
				foreach (KeyValuePair<string, string> keyValuePair in keyValuePairs)
				...
				//if (OracleService!=null)
				//{
				//    OracleService.UpdateDeliveryPriceAuditStatus(keyValuePairs, auditBy, auditStatus);
				//}
				return true;
			}
			catch (Exception ex)
		...
		public DataTable GetDeliveryPriceHistoryList(string lineNo)
		{
			if (OracleService != null)
			{
				return OracleService.GetDeliveryPriceHistoryList(lineNo);
			}

			XmlDocument xmlDoc = new XmlDocument();
			if (!string.IsNullOrEmpty(lineNo))
				xmlDoc = CreateXml(lineNo.Split(','));

			if (xmlDoc.DocumentElement != null && xmlDoc.DocumentElement.ChildNodes.Count > 0)
				return DisposeDeliveryPriceHistoryList(_deliveryPriceDao.GetDeliveryPriceHistoryList(xmlDoc.InnerXml));
			else
		...
		private DataTable DisposeDeliveryPriceHistoryList(DataTable dt)
		{
			...
			foreach (DataColumn dc in dt.Columns)
			{
				...
				if (name != "线路编号")
					name = name.Substring(0, 7) + name.Substring(9, name.Length - 9);//去除日
		...
		private XmlDocument CreateXml(string[] ids)
		{
			XmlDocument xmlDoc = new XmlDocument();
			XmlElement xe;
			...
			for (int i = 0; i < ids.Length; i++)
			{
				...
				xe.SetAttribute("v", ids[i].ToString());
		...
		public DataTable GetDeliveryPriceLog(string lineNo, string dateStr, string dateEnd, string distributionCode)
		{
			if (OracleService != null)
			{
				return OracleService.GetDeliveryPriceLog(lineNo, dateStr, dateEnd, distributionCode);
			}
		...
		public int AddEffectDeliveryPrice(FMS_CODLine codLine)
		{
            if (OracleService != null)
            {
                return OracleService.AddEffectDeliveryPrice(codLine);
            }
			...
			try
			{
				...
				using (IUnitOfWork work = TranScopeFactory.CreateUnit())
				{
					n = _deliveryPriceDao.AddEffectDeliveryPrice(codLine);
					if (n == 1)
				...
				//if (OracleService != null)
				//{
				//    OracleService.AddEffectDeliveryPrice(codLine);
				//}
				return n;
			}
			catch (Exception ex)
		...
		public FMS_CODLine GetListByEffectCodLineNo(string codLineNo)
		{
			if (OracleService != null)
			{
				return OracleService.GetListByEffectCodLineNo(codLineNo);
			}
		...
		public bool UpdateEffectCodLine(FMS_CODLine codLine)
		{
            if (OracleService != null)
            {
                return OracleService.UpdateEffectCodLine(codLine);
            }
			...
			try
			{
				...
				//if (OracleService != null)
				//{
				//    OracleService.UpdateEffectCodLine(codLine);
				//}
		...
		public bool UpdateEffectDeliveryPriceAuditStatus(IList<KeyValuePair<string, string>> keyValuePairs, string auditBy, int auditStatus)
		{
            if (OracleService != null)
            {
                return OracleService.UpdateEffectDeliveryPriceAuditStatus(keyValuePairs, auditBy, auditStatus);
            }
			try
			{
				foreach (KeyValuePair<string, string> keyValuePair in keyValuePairs)
				{
					...
					using (IUnitOfWork work = TranScopeFactory.CreateUnit())
					{
						...
						if (!AddDeliveryPriceLog(keyValuePair.Key, auditBy, "审核待生效，" + sbLog.ToString()))
						{
							return false;
						}
						work.Complete();
				...
				//if (OracleService != null)
				//{
				//    OracleService.UpdateEffectDeliveryPriceAuditStatus(keyValuePairs, auditBy, auditStatus);
				//}
				return true;
			}
			catch (Exception ex)
		...
		public bool ExportDeliveryPrice(DataTable dt, int createBy, out DataTable dtError, string distributionCode)
		{
            if (OracleService != null)
            {
                return OracleService.ExportDeliveryPrice(dt, createBy, out dtError, distributionCode);
            }
