...
namespace RFD.FMS.WEBLOGIC.COD
{
	...
	{
		public CODAccountService()
		{

		}

        private ICODAccountService OracleService;
        ...
        private string GetSearchExpressCompanyID(string expressCompanyId)
        {
            ...
            {
                expressCompanyChildsId = CreateHouseXml(_cODAccountDao.GetCompanyIdByRfd(), "ExpressCompanyID").InnerXml;
            }
            else
            {
                expressCompanyChildsId = CreateHouseXml(_cODAccountDao.GetCompanyIdByTopCODCompanyID(expressCompanyId), "ExpressCompanyID").InnerXml;
            }
		...
		public DataTable SearchUniteAccount(CODSearchCondition condition)
		{
            ...
            {
                return OracleService.SearchUniteAccount(condition);
            }

            condition.ExpressCompanyChilds = GetSearchExpressCompanyID(condition.ExpressCompanyID);
			condition.HouseD = CreateHouseXml(condition.HouseD).InnerXml;
			condition.HouseR = CreateHouseXml(condition.HouseR).InnerXml;
			condition.HouseV = CreateHouseXml(condition.HouseV).InnerXml;
		...
		public bool SaveAccount(CODSearchCondition searchCondition, DataTable AccountDetail, string createBy, out string AccountNo)
		{
            if (OracleService != null)
            {
                return OracleService.SaveAccount(searchCondition, AccountDetail, createBy, out AccountNo);
            }
			...
			GetAccountDetailTables(ref AccountDetail, int.Parse(searchCondition.ExpressCompanyID), int.Parse(searchCondition.MerchantID));
			try
			{
				using (IUnitOfWork work = TranScopeFactory.CreateUnit())
				{
					//统计表绑定结算单号
					if (!_cODAccountDao.ChanageCountAcountNO(searchCondition, createBy, AccountNo)) return false;
					//结算明细入库
					...
					//结算单入库				
					if (!_cODAccountDao.AddAccount(SplitWareHouse(searchCondition), createBy, AccountNo)) return false;
		...
		private CODSearchCondition SplitWareHouse(CODSearchCondition searchCondition)
		{
			searchCondition.HouseD = XmlToString(searchCondition.HouseD);
			searchCondition.HouseR = XmlToString(searchCondition.HouseR);
			searchCondition.HouseV = XmlToString(searchCondition.HouseV);

			return searchCondition;
		}

		private string XmlToString(string xmlString)
		{
			...
			{
				if(xn.Attributes["t"].Value.ToString()=="1")
					sbHouses.Append(xn.Attributes["v"].Value.ToString() + ",");
				else
					sbHouses.Append("S_" + xn.Attributes["v"].Value.ToString() + ",");
			}

			return sbHouses.ToString().TrimEnd(',');
		}
		...
		private void GetAccountDetailTables(ref DataTable AccountDetail, int expressCompanyId, int merchantid)
		{
			//linq 求某列和
			//var sum = AccountDetail.AsEnumerable().Sum(s => s.Field<Decimal>("DatumFare"));
			DataRow drNew;
			drNew = AccountDetail.NewRow();
			drNew["MerchantID"] = merchantid;
			drNew["ExpressCompanyID"] = expressCompanyId.ToString();
			drNew["AreaType"] = 999;
			drNew["Formula"] = "";
		...
		public bool DeleteAccountNo(IList<KeyValuePair<string, string>> keyValuePairs, string updateBy)
		{
            if (OracleService != null)
            {
                return OracleService.DeleteAccountNo(keyValuePairs, updateBy);
            }
		...
		public DataTable SearchAccount(string auditStatus, string expressCompanyId, string accountDateS, string accountDateE, string accountNo, string merchantId,ref PageInfo pi,bool isPage)
		{
            if(OracleService!=null)
            {
                return OracleService.SearchAccount(auditStatus, expressCompanyId, accountDateS, accountDateE, accountNo, merchantId, ref pi, isPage);
            }
		...
		public DataTable AccountSearchByNo(string accountNo, bool flag, out CODSearchCondition searchCondition)
		{
            if (OracleService != null)
            {
                return OracleService.AccountSearchByNo(accountNo, flag, out searchCondition);
            }
		...
		public DataTable SearchAreaFareByNo(string accountNo, out CODSearchCondition searchCondition)
		{
            if (OracleService != null)
            {
                return OracleService.SearchAreaFareByNo(accountNo, out searchCondition);
            }
		...
		private DataTable DisposeAreaData(DataTable dt)
		{
			...
			dr["Fare"] = decimal.Parse(dr["Allowance"].ToString()) + decimal.Parse(dr["KPI"].ToString())
										...
										+ decimal.Parse(dr["IntercityLose"].ToString()) + decimal.Parse(dr["IntercityLose"].ToString());
		...
		public CODSearchCondition GetSearchConditionByNo(string accountNo, bool flag)
		{
            if (OracleService != null)
            {
                return OracleService.GetSearchConditionByNo(accountNo, flag);
            }
		...
		private CODSearchCondition TransformConditionModel(DataTable dtCondition)
		{
			...
			sc.ExpressCompanyID = dtCondition.Rows[0]["ExpressCompanyID"].ToString();
			sc.CompanyName = dtCondition.Rows[0]["CompanyName"].ToString();
			...
			sc.MerchantID = dtCondition.Rows[0]["MerchantID"].ToString();
			sc.MerchantName = dtCondition.Rows[0]["MerchantName"].ToString();
		...
		public CODAccountDetail SearchAccountDetail(string accountDetailId)
		{
            if (OracleService != null)
            {
                return OracleService.SearchAccountDetail(accountDetailId);
            }
		...
		public bool UpdateAccountFee(CODAccountDetail accountDetail,string updateBy)
		{
            if (OracleService != null)
            {
                return OracleService.UpdateAccountFee(accountDetail, updateBy);
            }
		...
		public bool UpdateAccountAuditStatus(IList<KeyValuePair<string, string>> keyValuePairs,int auditStatus,string updateBy)
		{
            if (OracleService != null)
            {
                return OracleService.UpdateAccountAuditStatus(keyValuePairs, auditStatus, updateBy);
            }
			try
			{
				using (IUnitOfWork work = TranScopeFactory.CreateUnit())
		...
		public DataTable SearchDetail(string searchType,CODSearchCondition searchCondition,ref PageInfo pi)
		{
            if (OracleService != null)
            {
                return OracleService.SearchDetail(searchType, searchCondition, ref pi);
            }
		...
		public DataTable SearchExportDetail(string exportType, CODSearchCondition searchCondition)
		{
            if (OracleService != null)
            {
                return OracleService.SearchExportDetail(exportType, searchCondition);
            }
		...
		public DataTable GetErrorLog(CODSearchCondition condition)
		{
            if (OracleService != null)
            {
                return OracleService.GetErrorLog(condition);
            }
        ...
        public DataTable GetDifferenceData(CODSearchCondition condition)
        {
            if (OracleService != null)
            {
                return OracleService.GetDifferenceData(condition);
            }
