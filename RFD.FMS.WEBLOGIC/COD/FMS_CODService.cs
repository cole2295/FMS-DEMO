...
namespace RFD.FMS.WEBLOGIC.COD
{
	public class FMS_CODService : IFMS_CODService
	{
        ...
        private IFMS_CODService OracleService;
		...
		public bool ShouldUpdateForBack(DataRow waybill, out FMS_CODBaseInfo model,out string reason)
		{
			...
			///////////////////////////////////////////////////////
			//model.MediumID = Convert.ToInt64(waybill["MediumID"]); 
            model.MediumID = DataConvert.ToLong(waybill["ID"]);
			////
            if (DataConvert.ToInt(waybill["MerchantID"]) == 8
                || DataConvert.ToInt(waybill["MerchantID"]) == 9)
			{
		...
		public bool InsertForShip(FMS_CODBaseInfo model)
		{
            ...
            {
                return OracleService.InsertForShip(model);
            }

            if (model.OperateType == Convert.ToInt32(BizEnums.CODOperateType.Invalid))
            {
                //同步成功，同时更新同步标志位为1，否则回滚 oracle 执行
                UpdateDelete(@" 
                update fms_codbaseinfo set DeliverTime = sysdate
                ,DeliverDate = to_date(to_char(sysdate,'yyyy-mm-dd'),'yyyy-mm-dd') 
                where  DeliverTime is  null and waybillno =  " + model.WaybillNO);
            }
		...
		public bool UpdateForBack(FMS_CODBaseInfo model)
		{
            ...
            if (OracleService != null)
            {
                return OracleService.UpdateForBack(model);
            }
		...
		public bool AdvancedUpdateForBack(FMS_CODBaseInfo model)
		{
            if (OracleService != null)
            {
                return OracleService.AdvancedUpdateForBack(model);
            }
		...
		public string LmsSynFmsForShip(string topNumber, string synType)
		{
			...
			try
			{
                ...
                tip = DisposeLmsSynFmsForShip(ids);
                return string.IsNullOrEmpty(tip) ? "" : tip + "\r\n" + "<br/>" + "<br/>";
			}
			catch (Exception ex)
			{
        ...
        private string DisposeLmsSynFmsForShip(string ids)
        {
            var tip = "";
		...
		public int UpdateDelete(string sql)
		{
            ...
            if (OracleService != null)
            {
                return OracleService.UpdateDelete(sql);
            }
        ...
        {
            return DisposeDeliverTimeAndOutBountStation(ids);
        }
		...
		public string SynForDeliverTimeAndOutBountStation()
		{
			...
			try
			{
                ...
                tip= DisposeDeliverTimeAndOutBountStation(ids);
                return string.IsNullOrEmpty(tip) ? "" : tip + "\r\n";
			}
			catch (Exception ex)
			{
        ...
        private string DisposeDeliverTimeAndOutBountStation(string ids)
        {
            var tip = "";
            try
            {
