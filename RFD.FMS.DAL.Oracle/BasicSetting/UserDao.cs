 ...
 * 修改记录：
 */
    public class UserDao : OracleDao, IUserDao
    {
        ...
        {
            string strSql = string.Format("select employeeid from Employee  where employeecode='{0}'", EmployeeCode);
        ...
        public DataTable UserLogIn(Employee employee)
        {
            ...
            string sqlGetUserByIDAndName = "SELECT e.EmployeeID,e.EmployeeCode,e.EmployeeName,e.StationID,c.companyname,c.ExpressCompanyCode,c.DistributionCode FROM Employee e ,ExpressCompany c WHERE e.StationID=c.expressCompanyid and  e.EmployeeCode='{0}' AND e.PassWord='{1}' And e.IsDeleted=0";
            sqlGetUserByIDAndName = string.Format(sqlGetUserByIDAndName, employee.EmployeeCode, MD5.Encrypt(employee.PassWord));
            DataTable dataTable =
                OracleHelper.ExecuteDataset(Connection, CommandType.Text, sqlGetUserByIDAndName).Tables[0];
        ...
        public DataSet GetList(string strWhere)
        {
            ...
            strSql.Append(@"select EmployeeID 序号,EmployeeCode 员工编号,EmployeeOldCode 员工旧编号,
                                            ...
                                            (select CompanyName from ExpressCompany p where p.ExpressCompanyID=StationID) 所在站点或部门,Deposit 押金,PDACode PDA号码,POSCode POS机号码, 
        ...
        {
            string sqlGetEmployeeList = "SELECT EmployeeID as id,EmployeeCode,EmployeeOldCode,EmployeeName as name,Sex,PassWord,Address,CellPhone,PostID,eEmail,StationID,Deposit,PDACode,POSCode,Sorting,IsDeleted,CreatBy,CreateStationID,CreateTime,UpdateBy,UpdateTime,UpdateStationID FROM Employee  WHERE  IsDeleted=0";

            sqlGetEmployeeList += string.Format(" and StationID={0} ", departmentID);
            
            sqlGetEmployeeList += string.Format(" order by EmployeeName ");

            return OracleHelper.ExecuteDataset(Connection, CommandType.Text, sqlGetEmployeeList).Tables[0];
        }
        ...
        public DataTable GetUserRoles(int userId)
        {
            ...
            builder.Append("select rl.RoleID as id,rl.RoleName as name from Employee us inner join EmployeeRole userRole on us.EmployeeID = userRole.EmployeeID ");
            builder.Append(" inner join Role rl on userRole.RoleID = rl.RoleID where us.EmployeeID =");
            builder.Append(userId);

            DataSet dataSet = OracleHelper.ExecuteDataset(Connection, CommandType.Text, builder.ToString());
		...
		public IDictionary<int, string> GetUserRoles(string userCode)
		{
			...
			StringBuilder builder = new StringBuilder();
			builder.AppendFormat(@"SELECT r.RoleID, r.RoleName FROM ROLE r 
                             INNER JOIN EMPLOYEEROLE er 
                             ON r.RoleID = er.RoleID 
                             INNER JOIN EMPLOYEE e
                             ON e.EmployeeID = er.EmployeeID AND e.EmployeeCode = '{0}'", userCode);
			var ds = OracleHelper.ExecuteDataset(Connection, CommandType.Text, builder.ToString());
        ...
        public DataSet GetSampList(Employee model)
        {
            ...
            strSql.Append(@"select EmployeeID 序号,EmployeeCode 员工编码,EmployeeName 员工名称,
                                            (select CompanyName from ExpressCompany p where p.ExpressCompanyID=StationID) 所在部门
            ...
            strSql.Append(
                @" union  select EmployeeID 序号,EmployeeCode 员工编码,EmployeeName 员工名称,
                   (select CompanyName from ExpressCompany p where p.ExpressCompanyID=StationID) 所在部门 FROM Employee e
                   left join ExpressCompany p  on  p.ExpressCompanyID=StationID 
        ...
        public DataSet GetSampList(string employCode)
        {
            ...
            strSql.Append(@"select EmployeeID ,EmployeeCode ,EmployeeName 
                                            FROM Employee e  left join ExpressCompany p  on  p.ExpressCompanyID=StationID 
                        Where  p.CompanyFlag=4 ");
        ...
        {
            string sql = "update FMS_CODBaseInfo set IsChange=1 where INFOID in (select INFOID from FMS_CODBaseInfo baseInfo where IsChange=0 and RowNum <= 1000)";

            int count = OracleHelper.ExecuteNonQuery(Connection,CommandType.Text,sql);
        ...
        {
            string sql = "delete FMS_CODBaseInfo where INFOID in (select INFOID from FMS_CODBaseInfo where RowNum <= 200)";
