		...
		return this.each(function() {
			...
			function onSelect(cal) {
				...
				if (update && this_inputField) {
					...
					if(cal.inputClear){this_inputField.value = "";}/*检查清空标志inputClear，清空输入框*/
...
Calendar.cellClick = function(el, ev) {
	...
	if (typeof el.navtype == "undefined") {
		...
		switch (el.navtype) {
			...
			*/
			cal.inputClear = true;/*把清空标志置为真*/
			ev && cal.callHandler();
			Calendar.removeClass(el, "hilite");
		ev && cal.callCloseHandler();/*关闭*/
			return false;
			break;
...
Calendar.prototype.create = function (_par) {
	...
	hh("清空", 1, 400).ttip = Calendar._TT["INFO"];/*原为hh("?", 1, 400).ttip = Calendar._TT["INFO"];*/
...
Calendar.prototype._init = function (firstDayOfWeek, date) {
	...
	this.title.innerHTML = year + "年" + Calendar._MN[month]  ;
	this.onSetTime();
...
Calendar.prototype.showAtElement = function (el, opts) {
	...
	Calendar.continuation_for_the_fucking_khtml_browser = function() {
		...
		switch (valign) {
		    case "T": p.y -= h; break;
		    case "B": p.y += el.offsetHeight+3; break;//调低3像素，防止ie div覆盖不了select的bug.
...
// tooltips
Calendar._TT = {};
Calendar._TT["INFO"] = "清空输入框";/*原为帮助*/

Calendar._TT["ABOUT"] =
"日期选择:\n" +
"- 单击 \xab, \xbb 按钮分别选择上一年和下一年\n" +
"- 单击 " + String.fromCharCode(0x2039) + ", " + String.fromCharCode(0x203a) + " 按钮分别选择上一月和下一月\n" +
"- 在以上按钮上按下左键不放开可显示年月菜单.";
Calendar._TT["ABOUT_TIME"] = "\n\n" +
"时间选择:\n" +
"- 单击可增加时间\n" +
"- 按住Shift单击可减小时间\n" +
"- 您还可通过以按下左键并水平水来平拖动改变时间.";

Calendar._TT["PREV_YEAR"] = "上一年 (按下左键不放显示菜单)";
Calendar._TT["PREV_MONTH"] = "下一月 (按下左键不放显示菜单)";
Calendar._TT["GO_TODAY"] = "选择今天";
Calendar._TT["NEXT_MONTH"] = "下一月 (按下左键不放显示菜单)";
Calendar._TT["NEXT_YEAR"] = "下一年 (按下左键不放显示菜单)";
...
Calendar._TT["DAY_FIRST"] = "把 %s 显示在第一列";
...
Calendar._TT["TIME_PART"] = "按下左键并拖动可以改变值";
