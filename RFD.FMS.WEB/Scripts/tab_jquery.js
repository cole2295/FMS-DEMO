...
function TabView(option) {
	...
	function close(id) {
	    var index = jQuery.inArray(id, activateIds);
	    if (index != -1) {
	        activateIds.splice(index, 1);
	    }
	...
	this.add = function(option) {
	    ...
	    $.extend(op1, option);
	    if (jQuery.inArray(op1.id, activateIds) != -1) {
	        this.activate(op1.id);
	        return;
	    }
	    activateIds.push(op1.id);
	    $.extend(op1, option);
	    if (op1.title.length > 15) {
	        op1.title = op1.title.substring(0, 15);
	    }
