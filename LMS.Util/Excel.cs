...
namespace RFD.FMS.Util
{
	...
	public class Excel
	{
		public static readonly string TABLE_REGEX_TEXT = @"^<table";
		public static readonly string TR_REGEX_TEXT = @"<tr(.*?)>(.*?)</tr>";
		public static readonly string TH_REGEX_TEXT = @"<th(.*?)>(.*?)</th>";
		public static readonly string TD_REGEX_TEXT = @"<td(.*?)>(.*?)</td>";
		...
		public static DataSet ExcelToDataSet(string opnFileName)
		{
			...
			//以下引擎支持excel 2007
			string strConn = "Provider=Microsoft.ACE.OLEDB.12.0;Data Source=" + opnFileName + ";Extended Properties='Excel 12.0 Xml;HDR=YES'";
		...
		public static DataTable ExcelToDataSetFor03And07Ex(string filePath, DataTable template)
		{
			SetExcelConnectionStrings();
			...
			using (OleDbConnection connection = OleDbConnectionOpened(filePath))
			{
				...
				foreach (string sheetName in sheetNames)
				{
					...
					if (template != null)
					{
						...
						if (ExcelHelper.IsTemplateFormat(data, template))
							break;
					}
				}
		...
		{
			if (gridForm == null || gridForm.Rows.Count == 0)
			{
				return false;
			}
		...
		public static bool OutputXLSFromDataTable(string[] columns, DataTable dt, string strPath)
		{
			...
			try
			{
				...
				foreach (DataRow dr in dt.Rows)
				{
					...
					foreach (DataColumn col in dt.Columns)
					{
						excel.WriteString(rows, cols, dr[col].ToString().Replace("&nbsp;", " "));
						cols++;
					}
					rows++;
				}
		...
		/// <param name="saveFileDialog">对话框</param>
		/// <returns></returns>
		public static bool OutputXLSFromDataTable(string[] columns, DataTable dt, string strPath, string title)
		{
			if (Common.DataTableIsEmpty(dt))
			{
				return false;
			...
			try
			{
				...
				ExcelWriter excel = new ExcelWriter(strPath);
				excel.BeginWrite();
				excel.WriteString(0, 3, title);
				short cols = 0;
				if (columns == null || columns.Length == 0)
				{//若没有传列名，则以dt的列名做为Excel的列名
					foreach (DataColumn col in dt.Columns)
					{
						excel.WriteString(1, cols, col.ColumnName);
						cols++;
					}
				}
				else
				{
					foreach (string column in columns)
					{
						excel.WriteString(1, cols, column);
						cols++;
					}
				}
		...
		public static bool OutputXLSFromDataGridView(DataGridView grid, string fileName, bool visibleOnly, bool includeNewRow)
		{
			if (grid == null || grid.Rows.Count == 0)
			{
				return false;
		...
		/// </summary>
		/// <param name="grid">DataGridView</param>
		/// <param name="fileName">xls文件路径</param>
		/// <param name="visibleOnly">只导出可见的列</param>
		/// <param name="includeNewRow">是否导出新行</param>
		/// <param name="startColInXls">写入xls的开始列</param>
		/// <param name="startRowInXls">写入xls的开始行</param>
		public static bool OutputXLSFromDataGridView(ExcelWriter excel, DataGridView grid, string fileName, bool visibleOnly, bool includeNewRow, short startColInXls, short startRowInXls)
		{
			if (grid == null || grid.Rows.Count == 0)
			{
				return false;
			...
			try
			{
				//ExcelWriter excel = new ExcelWriter(fileName);
				//excel.BeginWrite();
				short cols = startColInXls;
				short rows = startRowInXls;
				foreach (DataGridViewColumn col in grid.Columns)//写表头
				{
					if (visibleOnly && !col.Visible) continue;
					string column = col.HeaderText;
					excel.WriteString(rows, cols, column);
					cols++;
				}
				rows++;//表头写完，加一行
				foreach (DataGridViewRow gr in grid.Rows)
		...
		/// </summary>
		/// <param name="array">二维数组</param>
		/// <param name="fileName">xls文件名，包含路径</param>
		/// <param name="startColInXls">写入xls的开始列</param>
		/// <param name="startRowInXls">写入xls的开始行</param>
		public static void OutputXLSFromArray(ExcelWriter excel, string[,] array, string fileName, short startColInXls, short startRowInXls)
		{
			...
			int rowLength = array.GetUpperBound(0) + 1;
			int columnLength = array.GetUpperBound(1) + 1;
			short colInXls = startColInXls;
			short rowInXls = startRowInXls;
			while (rowsInArray < rowLength)
			{
				colsInArray = 0;
				colInXls = startColInXls; ;
				while (colsInArray < columnLength)
				{
					excel.WriteString(rowInXls, colInXls, array[rowsInArray, colsInArray]);
					colsInArray++;
					colInXls++;
				}
				rowInXls++;
		...
		/// 格式化非标准的Excel文件
		/// </summary>
		/// <param name="fileName">非标准的Excel文件路径</param>
		/// <returns></returns>
		public static DataTable FormatExcel2Datatable(string fileName)
		{
			using (StreamReader reader = new StreamReader(File.Open(fileName, FileMode.Open)))
			{
				var data = default(DataTable);
				var content = reader.ReadToEnd();
				var table = new Regex(TABLE_REGEX_TEXT, RegexOptions.IgnoreCase);
				var tableMatchs = table.Matches(content);
				if (!content.IsNullData())
				{
					if (tableMatchs.IsNullData() || tableMatchs.Count == 0)
						return null;
					data = new DataTable();
					var tr = new Regex(TR_REGEX_TEXT, RegexOptions.IgnoreCase);
					var th = new Regex(TH_REGEX_TEXT, RegexOptions.IgnoreCase);
					var td = new Regex(TD_REGEX_TEXT, RegexOptions.IgnoreCase);
					var matches = tr.Matches(content);
					if (matches == null || matches.Count <= 0) return null;
					for (var i = 0; i < matches.Count; i++)
					{
						if (i == 0)
						{  //获取表头
							var headers = matches[0].Groups[matches[0].Groups.Count - 1].Value;
							var ths = th.Matches(headers);
							foreach (Match m in ths)
							{
								data.Columns.Add(m.Groups[m.Groups.Count - 1].Value);
							}
							continue;
						}
						//获取内容
						var row = matches[i].Groups[matches[i].Groups.Count - 1].Value;
						var tds = td.Matches(row);
						var dr = data.NewRow();
						for (var j = 0; j < tds.Count; j++)
						{
							dr[j] = tds[j].Groups[tds[j].Groups.Count - 1].Value;
						}
						data.Rows.Add(dr);
					}
				}
