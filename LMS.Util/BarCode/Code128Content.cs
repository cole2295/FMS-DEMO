...
namespace RFD.FMS.Util.BarCode
{
    public enum CodeSet
    {
        CodeA,
    ...
    public class Code128Content
    {
        ...
        public Code128Content(string AsciiData, CodeSet cs)
        {
            if (cs == CodeSet.CodeC)
            {
                mCodeList = StringToCode128C(AsciiData);
            }
            else
            {
        ...
        private int[] StringToCode128(string AsciiData)
        {
            ...
            ArrayList codes = new ArrayList(asciiBytes.Length + 3); // assume no codeset changes, account for start, checksum, and stop
        ...
        /// <returns></returns>
        private int[] StringToCode128C(string inputData)
        {
            if (inputData.Length % 2 != 0)
            {
                throw new Exception("Code128C要求字符长度为偶数的阿拉伯数字");
            }
            int len = inputData.Length / 2 + 3;
            ArrayList result = new ArrayList(len);
            result.Add(105);
            for (int i = 0; i < inputData.Length; i += 2)
            {
                int tmp;
                if (!int.TryParse(inputData.Substring(i, 2), out tmp))
                {
                    throw new Exception("Code128C要求阿拉伯数字");
                }
                result.Add(tmp);
            }
            int checksum = (int)(result[0]);
            for (int i = 1; i < result.Count; i++)
            {
                checksum += i * (int)(result[i]);
            }
            result.Add(checksum % 103);
            result.Add(106);
            int[] codes = result.ToArray(typeof(int)) as int[];
            return codes;
        }
