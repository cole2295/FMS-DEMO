...
namespace ServiceForCodAccount.BLL
{
	...
	/// 上门退统计
	/// </summary>
	public class VisitReturnsStats : IService
	{
		#region IService 成员
		...
		private void StatsToday()
		{
			try
			{
				List<CodStatsLogModel> codStatsLogListTmp = cODBaseInfoService.GetVisitToDayStatsInfo(_accountDate);
			...
			{
				Common.Common.SendLogEmail(_taskModel, "上门退按天统计当前获取异常-1:", ex.Message, ex);
			}
		}
		...
		private void StatsHistory()
		{
			try
			{
                List<CodStatsLogModel> codStatsLogListTmp = cODBaseInfoService.GetStatsLogError((int)EnumCsType.T3, _accountDate.ToShortDateString(),Common.Common.AccountDays);
				List<CodStatsLogModel> codStatsLogList = Collate(codStatsLogListTmp);//待统计对象列表
				if (codStatsLogList == null || codStatsLogList.Count <= 0)
					return;
				foreach (CodStatsLogModel codStatsLog in codStatsLogList)
			...
			{
				Common.Common.SendLogEmail(_taskModel, "上门退按天统计历史获取异常-1:", ex.Message, ex);
			}
		}
		...
		private void StatsByDay(CodStatsLogModel codStatsLog)
		{
			try
			{
                List<CodStatsModel> codStatsListTmp = cODBaseInfoService.GetVisitAccountByDay(codStatsLog);//获取统计
				if (codStatsListTmp == null || codStatsListTmp.Count <= 0)
				{
					Common.Common.SendLogEmail(_taskModel, "上门退按天统计入库异常-4:", "日期：" + codStatsLog.StatisticsDate +
                                                " 配送商：" + codStatsLog.ExpressCompanyID + " 仓库：" + codStatsLog.WareHouseID + " " + codStatsLog.MerchantID, null);
					return;
				}
				...
				foreach (CodStatsModel codStats in codStatsListTmp)
				{
					...
					if (codStatsList.Count == _taskModel.insertNum)
					{
						try
						{
							cODBaseInfoService.InsertVisitReturnsAccount(codStatsList, codStatsLog.StatisticsDate.ToShortDateString());
						}
						catch (Exception ex)
						{
                            Common.Common.SendLogEmail(_taskModel, "上门退按天统计入库异常-3:", ex.Message + "\n" + codStatsLog.ExpressCompanyID + " " + codStatsLog.WareHouseID 
                                + " " + codStatsLog.MerchantID, ex);
						}
						codStatsList.Clear();
					}
				}
				...
				if (codStatsList.Count > 0)
				{
					...
					{
                        Common.Common.SendLogEmail(_taskModel, "上门退按天统计入库异常-2:", ex.Message + "\n" + codStatsLog.ExpressCompanyID + " " + codStatsLog.WareHouseID 
                            + " " + codStatsLog.MerchantID, ex);
					}
				}
			}
			...
			{
                Common.Common.SendLogEmail(_taskModel, "上门退按天统计入库异常-1:", ex.Message + "\n" + codStatsLog.ExpressCompanyID + " " + codStatsLog.WareHouseID 
                    + " " + codStatsLog.MerchantID, ex);
			}
		}
		...
		private List<CodStatsLogModel> Collate(List<CodStatsLogModel> codStatsLogListTmp)
		{
			try
			{
				...
				int AllCount = 0;
				int FareCount = 0;
				string errorStr = "配送商：{0} 仓库：{1} 统计时间：{2} 相差 {3} 量";
				...
				foreach (CodStatsLogModel codStatsLog in codStatsLogListTmp)
				{
					...
					if (DateTime.TryParse(codStatsLog.StatisticsDate.ToString(), out _accountDate))
					{
						if (codStatsLog.FormCount == 0)//当前的无序查询总数，历史的查询总数
                            AllCount = cODBaseInfoService.GetVisitAllCountByExpressWareHose(codStatsLog);
						else
							AllCount = codStatsLog.FormCount;

                        FareCount = cODBaseInfoService.GetVisitFareCountByExpressWareHouse(codStatsLog);
						...
						else
						{
							...
							sbError.Append(string.Format(errorStr, codStatsLog.ExpressCompanyID, codStatsLog.WareHouseID,
				...
				if (sbError.Length > 0)
				{
					Common.Common.SendLogEmail(_taskModel, "上门退按天统计校对-2:", sbError.ToString(), null);
				}
				return codStatsLogList;
			}
			catch (Exception ex)
			{
				Common.Common.SendLogEmail(_taskModel, "上门退按天统计校对-1:", ex.Message, ex);
				return null;
			}
		}
	}
