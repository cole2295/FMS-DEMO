...
namespace ServiceForCodAccount.BLL
{
	...
	/// 拒收运费明细
	/// </summary>
	public class ReturnsDetail : IService
	{
		...
		private void DetailProcess()
		{
			try
			{
                IList<FMS_CODBaseInfo> details = cODBaseInfoService.GetReturnDetails(Common.Common.AccountDays, int.Parse(_taskModel.topNum), _taskModel.SyncStartTime);
                ...
                foreach (FMS_CODBaseInfo d in details)
				{
					...
					if (detailInsert.Count == _taskModel.insertNum)//有剩余情况
					{
						...
						{
							Common.Common.SendLogEmail(_taskModel, "拒收明细服务异常-4:", ex.Message, ex);
						}
						detailInsert.Clear();
					}
				}
				if (detailInsert.Count > 0)
				{
					...
					{
						Common.Common.SendLogEmail(_taskModel, "拒收明细服务异常-3:", ex.Message, ex);
					}
				}
				...
				if (detailError.Count > 0)
				{
                    ...
                    if (sbErrorMsg.Length > 0)
					    Common.Common.SendLogEmail(_taskModel, "拒收明细服务异常-2:", sbErrorMsg.ToString(), null);
				}
				Thread.Sleep(1000);
			}
			catch (Exception ex)
			{
				Common.Common.SendLogEmail(_taskModel, "拒收明细服务异常:", ex.Message, ex);
			}
		}
        ...
        private void UpdateCodFare(List<FMS_CODBaseInfo> detailInsert)
		{
			...
			{
				Common.Common.SendLogEmail(_taskModel, "拒收明细服务异常-1:", sb.ToString(), null);
			}
		}
