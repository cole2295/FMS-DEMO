...
{
    public class M2S_FMS_IncomeVisitReturnsCount : SyncBase
    {
        private SyncSelectParam ssParam = SyncSelectParamByConfig.Instance.GetParam("FMS_IncomeVisitReturnsCount");

        protected override DataSet GetSyncData()
        {
            SyncDao _dao = new SyncDao();

            DataSet ds = _dao.GetDataSet(DBStr.DbFromConnString, string.Format(SqlCollectionM2S.sqlIncomeVisitReturnsCount, ssParam.Where, ssParam.TopN));
        ...
        protected override SyncTable GetSynStructFrom()
        {
            string userSql = @" UPDATE o SET o.ischange = 0
                                from FMS_IncomeVisitReturnsCount as o join {0} as f on o.CountID = f.CountID
                                where o.ischange=1";
        ...
        {
            SyncTable t = new SyncTable("FMS_IncomeVisitReturnsCount", HowTableUpdate.TableInsertAndUpdate);
            t.Cols.Add(new SyncCol("CountID","bigint", true));
        ...
        {
            return "M2S_FMS_IncomeVisitReturnsCount";
        }
    }
}
