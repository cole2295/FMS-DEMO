...
{
    public class M2S_FMS_IncomeReturnsCount : SyncBase
    {
        private SyncSelectParam ssParam = SyncSelectParamByConfig.Instance.GetParam("FMS_IncomeReturnsCount");

        protected override DataSet GetSyncData()
        {
            SyncDao _dao = new SyncDao();

            DataSet ds = _dao.GetDataSet(DBStr.DbFromConnString, string.Format(SqlCollectionM2S.sqlIncomeReturnsCount, ssParam.Where, ssParam.TopN));
        ...
        protected override SyncTable GetSynStructFrom()
        {
            string userSql = @" UPDATE o SET o.ischange = 0
                                from FMS_IncomeReturnsCount as o join {0} as f on o.CountID = f.CountID
                                where o.ischange=1";
        ...
        {
            SyncTable t = new SyncTable("FMS_IncomeReturnsCount", HowTableUpdate.TableInsertAndUpdate);
            t.Cols.Add(new SyncCol("CountID","bigint", true));
        ...
        {
            return "M2S_FMS_IncomeReturnsCount";
        }
    }
}
