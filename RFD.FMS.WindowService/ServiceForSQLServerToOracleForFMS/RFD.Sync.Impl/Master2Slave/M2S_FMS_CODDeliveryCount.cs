...
{
    public class M2S_FMS_CODDeliveryCount : SyncBase
    {
        private SyncSelectParam ssParam = SyncSelectParamByConfig.Instance.GetParam("FMS_CODDeliveryCount");

        protected override DataSet GetSyncData()
        {
            SyncDao _dao = new SyncDao();

            DataSet ds = _dao.GetDataSet(DBStr.DbFromConnString, string.Format(SqlCollectionM2S.sqlCODDeliveryCount, ssParam.Where, ssParam.TopN));
        ...
        protected override SyncTable GetSynStructFrom()
        {
            string userSql = @" UPDATE o SET o.ischange = 0
                                from FMS_CODDeliveryCount as o join {0} as f on o.AccountID = f.AccountID
                                where o.ischange=1";
        ...
        {
            SyncTable t = new SyncTable("FMS_CODDeliveryCount", HowTableUpdate.TableInsertAndUpdate);
            t.Cols.Add(new SyncCol("AccountID", "bigint", true));
            ...
            t.Cols.Add(new SyncCol("DeliveryType", "tinyint"));
        ...
        {
            return "M2S_FMS_CODDeliveryCount";
        }
    }
}
