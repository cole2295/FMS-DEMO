...
{
    public class M2S_FMS_CODLineWaitEffect : SyncBase
    {
        private SyncSelectParam ssParam = SyncSelectParamByConfig.Instance.GetParam("FMS_CODLineWaitEffect");

        protected override DataSet GetSyncData()
        {
            SyncDao _dao = new SyncDao();

            DataSet ds = _dao.GetDataSet(DBStr.DbFromConnString, string.Format(SqlCollectionM2S.sqlCODLineWaitEffect, ssParam.Where, ssParam.TopN));
        ...
        protected override SyncTable GetSynStructFrom()
        {
            string userSql = @" UPDATE o SET o.ischange = 0
                                from FMS_CODLineWaitEffect as o join {0} as f on o.LineID = f.LineID
                                where o.ischange=1";
        ...
        {
            SyncTable t = new SyncTable("FMS_CODLineWaitEffect", HowTableUpdate.TableInsertAndUpdate);
            t.Cols.Add(new SyncCol("LineID","bigint", true));
            ...
            t.Cols.Add(new SyncCol("EffectDate", "date"));
        ...
        {
            return "M2S_FMS_CODLineWaitEffect";
        }
    }
}
