...
using RFD.FMS.ServiceImpl.PMSOpenService;
...
namespace RFD.FMS.ServiceImpl.FinancialManage
{
    public class DeductService : IDeductService
    {
        ...
        private bool DealExpressReceive()
        {
            ...
            using (IUnitOfWork work = TranScopeFactory.CreateOracleUnit())
            {
                foreach (var item in list)
                {
                    ...
                    //if (waybills.Contains(item.WaybillNO + "_" + (int)BizEnums.enumDeductType.ExpressReceive)) continue;

                    //waybills.Add(item.WaybillNO + "_" + (int)BizEnums.enumDeductType.ExpressReceive);
                    ...
                    reductSubInfo.AreaCommission = AreaCommission;
                        // GetStationAreaDeduct(item.ReceiveStationID, item.AreaReceiveDeduct);//AreaCommission; 
                    reductSubInfo.SumCommission = reductSubInfo.BasicCommission + reductSubInfo.AreaCommission +
                                                  reductSubInfo.WeightCommission;
                    reductSubInfo.CreateTime = DateTime.Now;
                    reductSubInfo.UpdateTime = DateTime.Now;
                    reductSubInfo.IsDeleted = 0;
                    reductSubInfo.AdjustCommission = item.AdjustCommission ?? 0;
        ...
        private bool DealExpressSend()
        {
            ...
            using (IUnitOfWork work = TranScopeFactory.CreateOracleUnit())
            {
                foreach (var item in list)
                {
                    ...
                    //if (waybills.Contains(item.WaybillNO + "_" + (int)BizEnums.enumDeductType.ExpressSend)) continue;

                    //waybills.Add(item.WaybillNO + "_" + (int)BizEnums.enumDeductType.ExpressSend);
                    ...
                    reductSubInfo.SumCommission = reductSubInfo.BasicCommission + reductSubInfo.AreaCommission +
                                                  reductSubInfo.WeightCommission;
                    reductSubInfo.CreateTime = item.BackStationTime ?? DateTime.Now;
                    reductSubInfo.UpdateTime = DateTime.Now;
                    reductSubInfo.IsDeleted = 0;
                    reductSubInfo.AdjustCommission = item.AdjustCommission ?? 0;
        ...
        private bool DealProjectSend()
        {
            ...
            using (IUnitOfWork work = TranScopeFactory.CreateOracleUnit())
            {
                ...
                foreach (var item in list)
                {
                    ...
                    //if (waybills.Contains(item.WaybillNO + "_" + (int)BizEnums.enumDeductType.ProjectSend)) continue;
                    
                    //waybills.Add(item.WaybillNO + "_" + (int)BizEnums.enumDeductType.ProjectSend);
                    ...
                    reductSubInfo.DeliverUser = item.DeliverManID;
                    reductSubInfo.BasicCommission = basicCommission;
                    reductSubInfo.AreaCommission = AreaCommission; //GetStationAreaDeduct(item.DeliverStationID, item.AreaSendDeduct);//AreaCommission; 
        ...
        private void SendEmptyGoodsMail(FMS_DeductBaseInfo deductInfo)
        {
            ...
            mail.SendMailToUser("提成货物品类异常", builder.ToString(), "zhaojunb@vancl.cn;zuobin@vancl.cn;zhangjunb@vancl.cn;zhangping@vancl.cn;xukejun@vancl.cn;gaopengxiang@vancl.cn");
        }
        ...
        private void SendStationCategoryNotSetMail(int stationId, string categoryCode)
        {
            ...
            mail.SendMailToUser("站点货物品类", builder.ToString(), "xukejun@vancl.cn;zhangjunb@vancl.cn;zhangping@vancl.cn;gaopengxiang@vancl.cn");
        }

        public decimal GetStationAreaDeduct1(int? stationId, decimal? areaDeduct)
        {
