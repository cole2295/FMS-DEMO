...
namespace RFD.FMS.ServiceImpl.BasicSetting
{
    public class AreaExpressLevelService : IAreaExpressLevelService
    {
				...
				using (IUnitOfWork work = TranScopeFactory.CreateOracleUnit())
				{
					...
					foreach (AreaExpressLevel areaExpressLevel in areaExpressLevels)
					{
						...
						else
						{
							...
							areaExpressLevelLog.Enable = int.Parse(dt.Rows[0]["IsEnable"].ToString());
        ...
        {
            return _areaExpressLevelDao.SearchSecondAreaType(areaID, areaType, stationID, merchantID, distributionCode,ref pi); 
        }
    }
}
